...
cap = cv2.VideoCapture('baby2.gif')#Enter path to your gif file here


while 1:
    ret, frame = cap.read()
    ret2,frame2 = cap2.read()
    frame2 = cv2.resize(frame2, None, fx=0.75, fy=0.75, interpolation = cv2.INTER_LINEAR)
    ...
    if ret:
        ...
        swapped = swappy(frame, frame2)
        swapped = cv2.resize(swapped, None, fx=1.5,fy=1.5, interpolation=cv2.INTER_LINEAR)
        cv2.startWindowThread()
        cv2.namedWindow('SWAP')
        cv2.imshow('SWAP',swapped)
    else:
        cap = cv2.VideoCapture('baby2.gif')
    if cv2.waitKey(1) == 13:
        cv2.imwrite('gifSwap.jpg',swapped)
        break
